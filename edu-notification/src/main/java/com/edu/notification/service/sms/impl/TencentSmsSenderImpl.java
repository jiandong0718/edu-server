 ...
 * 注意：这是Mock实现，用于演示和测试
 * 真实环境需要集成腾讯云SMS SDK：
 * 1. 添加Maven依赖：com.tencentcloudapi:tencentcloud-sdk-java-sms
 * 2. 使用腾讯云SDK的Client进行真实发送
 * 3. 处理真实的响应和异常
 */
@Slf4j
@Component("tencentSmsSender")
public class TencentSmsSenderImpl implements SmsSender {
    ...
    public SmsResult sendSms(String phone, String content) {
        log.info("腾讯云短信发送 - 手机号: {}, 内容: {}", phone, content);

        if (smsProperties.getMockEnabled()) {
            return mockSend(phone, content);
        }

        // TODO: 真实环境集成腾讯云SMS SDK
        // 示例代码：
        // try {
        //     Credential cred = new Credential(
        //         smsProperties.getTencent().getSecretId(),
        ...
        //     req.setSmsSdkAppId(smsProperties.getTencent().getSdkAppId());
        //     req.setSignName(smsProperties.getTencent().getSignName());
        ...
        //         return SmsResult.success(resp.getSendStatusSet()[0].getSerialNo(), "0.05");
        //     } else {
        //         return SmsResult.fail(resp.getSendStatusSet()[0].getMessage());
        //     }
        // } catch (Exception e) {
    ...
    public List<SmsResult> sendBatchSms(List<String> phones, String content) {
        log.info("腾讯云批量短信发送 - 手机号数量: {}, 内容: {}", phones.size(), content);
    ...
    public SmsResult sendTemplateSms(String phone, String templateCode, Map<String, String> params) {
        log.info("腾讯云模板短信发送 - 手机号: {}, 模板: {}, 参数: {}", phone, templateCode, params);
    ...
    public List<SmsResult> sendBatchTemplateSms(List<String> phones, String templateCode, Map<String, String> params) {
        log.info("腾讯云批量模板短信发送 - 手机号数量: {}, 模板: {}, 参数: {}", phones.size(), templateCode, params);
    ...
    public String querySendStatus(String thirdPartyId) {
        log.info("腾讯云查询短信发送状态 - 消息ID: {}", thirdPartyId);
    ...
    private SmsResult mockSend(String phone, String content) {
        // 模拟发送成功
        String thirdPartyId = "TENCENT-" + UUID.randomUUID().toString().substring(0, 8);
        log.info("Mock腾讯云短信发送成功 - 手机号: {}, 消息ID: {}", phone, thirdPartyId);
    ...
    private String buildContentFromTemplate(String templateCode, Map<String, String> params) {
        // 简单的模板替换逻辑
        String content = "【" + smsProperties.getTencent().getSignName() + "】";
